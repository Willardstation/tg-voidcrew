...
/obj/machinery/power/shuttle_engine/electric/update_engine()
	. = ..()
	if(!.)
		return FALSE
...
/obj/machinery/power/shuttle_engine/electric/on_construction()
	. = ..()
	connect_to_network()

/obj/machinery/power/shuttle_engine/electric/burn_engine(percentage = 100)
	. = ..()
...
/obj/machinery/power/shuttle_engine/electric/return_fuel()
	if(length(powernet?.nodes) == 2)
		for(var/obj/machinery/power/smes/S in powernet.nodes)
			return S.charge
	return newavail()

/obj/machinery/power/shuttle_engine/electric/return_fuel_cap()
	if(length(powernet?.nodes) == 2)
		for(var/obj/machinery/power/smes/S in powernet.nodes)
			return S.capacity
